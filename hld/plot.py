...
def _add_phase_legend_outside(
    ...
    ax,
    theme="dark",
    wheel_diameter_in=1.1,   # <-- now specified in inches, not fractions
    right_margin_in=0.35,    # distance from right figure edge (inches)
    center_text="φ",
    label_fs=9,
    center_fs=12,
):
    """
    Outside-right phase wheel that stays perfectly circular across resizes.
    Sizing is expressed in INCHES so figure resizing never distorts it.
    """
    ...
    # convert wheel size to figure fraction units
    wheel_frac = wheel_diameter_in / fig_w
    margin_frac = right_margin_in / fig_w

    # y-position: center relative to plot's vertical center
    pos = ax.get_position()
    ...
    axl.set_axis_off()
    axl.set_aspect("equal", adjustable="box")  # <-- ensures perfect circle

    # draw wheel ring
    for deg in range(360):
        color = hsv_to_rgb([deg / 360.0, 1.0, 1.0])
        axl.add_patch(Wedge((0.5, 0.5), 0.48,
                            deg, deg + 1, width=0.18,
                            facecolor=color, edgecolor=color, linewidth=0))

    axl.text(0.5, 0.5, center_text, color=txt,
...
def plot_hld(
    ...
    # --- Ensure legend gets enough physical width so circle is not squashed ---
    legend_needed_in = 1.2  # inches reserved for legend (works well visually)
    ...
    if show_legend and legend_kind!="none":
        if current_fig_w - (fig_w - legend_needed_in) < legend_needed_in:
            # enlarge figure width to guarantee space
            new_fig_w = fig_w + legend_needed_in
            fig.set_size_inches(new_fig_w, fig.get_size_inches()[1])
            fig.canvas.draw()

    # after resizing, compute legend lane fraction
    legend_space = legend_needed_in / fig.get_size_inches()[0]
    title_band   = 0.12
    caption_band = 0.16 if caption else 0.00

    # --- Two-column layout: left = plot, right = legend lane (fixed inch width) ---
    legend_width_in = 1.4  # width of phase wheel lane in inches
    fig_w, fig_h = fig.get_size_inches()
    ...
    if show_metrics:
        ...
        title_text = f"Global coherence: {m['global_coherence_spectrum']:.3f} |  Entanglement visibility: {m['entanglement_visibility_index']:.3f}"
        title_ax.text(0.5, 0.5, title_text, ha="center", va="center",
                      fontsize=11, color=label_color)

    # ---------------- Caption Band (Your correct layout) --------------------
    if caption:
        fig.canvas.draw()
        axpos = ax.get_position()
        cap_ax = fig.add_axes([
            axpos.x0,
            0.04,                   # always bottom-anchored
            axpos.width,
            caption_band - 0.04
        ])
        cap_ax.set_axis_off()
    ...
    # ✅ PLACE LEGEND AFTER ALL RESIZING IS FINISHED
